...
SW, SH = 1080, 720                          # Setting up
SC = pg.display.set_mode((SW,SH))           # the
pg.display.set_caption("AI Project Box Game")       # screen
...
moveLimit = 300         # Frame limit for free movement of boxes
boxCount = 200        # Amount of boxes for each generation
generationCount = 0
...
class DNA(object):                                                  # DNA object for genetic algorithm
    ...
    #def crossover(self, partner):
       # crossover_point = random.randint(0, len(self.array))
        #return DNA(partner.array[:crossover_point] + self.array[crossover_point:])
...
class SmartBox(object):                         # Our smart box object
    def __init__(self, dna=None):               # If initialized with a DNA, use that dna, else, create new.
        ...
        self.vel.xy = 0, 0
        self.velLimit = 6
        self.burstColor = pg.Color("white")
        self.burstSize = 10
        self.fitness = 0
...
while True:                             # To keep the window open.
    ...
    pg.draw.circle(SC, pg.Color("white"), (int(finish.x), int(finish.y)), 20)       # Finally, draw the end goal.
